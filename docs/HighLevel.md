...
must manipulate the config objects *directly* if you need to access thier more
extended features.   
...
The `isObfuscating` switch toogles whether code obfuscation is employed by a process
using the config factory (based opon context).  
...
This attribute is a relatively sophistated component used for   
producing a given [Stand Alone Executable](LowLevel.md#stand-alone-executables).
...
class which allows for programatic generation and manipulation of such
...
Based upon context, this attribute is used to embeded (or bundle) an icon for
